...
(*
But  du jeu : ici on ne va pas avoir a coder en Ocaml, mais plutot a
creer des machines de T pour pouvoirr repondre aux questions du prof,
du coup ca devrait etre un peu plus faisable
Par exemple pour la 1e question, la machine c'estune bande d entree une bande de sortie et une
bande qui contient la pile
max -> je peux essayer d'en implémenter
